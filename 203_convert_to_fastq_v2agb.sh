...
mkdir -p data/map_fastq_v2agb
for genome in "${genomes[@]}"
do
  echo "${genome}"

  samtools view -@ 16 -b "data/only-map_v2agb/${genome}.sam" > "data/only-map_v2agb/${genome}_unsorted.bam"
  exit
  samtools sort -@ 16 "data/only-map_v2agb/${genome}_unsorted.bam" -o "data/only-map_v2agb/${genome}.bam"
  exit
  samtools fastq -@ 16 "data/only-map_v2agb/${genome}.bam" > "data/map_fastq_v2agb/${genome}.fq"
  exit
done
